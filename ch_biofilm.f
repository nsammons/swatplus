...
!      real :: thrk5 = 1.047, thrk6 = 1.0, thrs6 = 1.024, thrs7 = 1.0
       ...
       wtrin = 0.
       wtrin = ob(icmd)%hd(1)%flo * (1. - rnum1)
...
!! concentrations
         ...
         cinn = 0.
         if (wtrin > 0.001) then
         chlin = 1000. * ob(icmd)%hd(1)%chla * (1. - rnum1) / wtrin
         algin = 1000. * chlin / ch_nut(jnut)%ai0        !! QUAL2E equation III-1
         orgnin = 1000. * ob(icmd)%hd(1)%orgn * (1. - rnum1) / wtrin
         ammoin = 1000. * ob(icmd)%hd(1)%nh3 * (1. - rnum1) / wtrin
         nitritin = 1000. * ob(icmd)%hd(1)%no2 * (1. - rnum1) / wtrin
         nitratin = 1000. * ob(icmd)%hd(1)%no3 * (1. - rnum1) / wtrin
         orgpin = 1000. * ob(icmd)%hd(1)%sedp * (1. - rnum1) / wtrin
         dispin = 1000. * ob(icmd)%hd(1)%solp * (1. - rnum1) / wtrin
         cbodin = 1000. * ob(icmd)%hd(1)%cbod * (1. - rnum1) / wtrin
         disoxin= 1000. * ob(icmd)%hd(1)%dox * (1. - rnum1) / wtrin
         end if
         ...
         nh3con = (ammoin * wtrin + ch(jrch)%ammonian * rchwtr) /       
     &                                                           wtrtot
         no2con = (nitritin * wtrin + ch(jrch)%nitriten * rchwtr) /     
     &                                                          wtrtot
         no3con = (nitratin * wtrin + ch(jrch)%nitraten * rchwtr) /     
     &                                                          wtrtot
...
!! oxygen calculations
	   ...
	   if (ch(jrch)%rch_cbod > dcoef * cbodcon)  ch(jrch)%rch_cbod =       
     &                           dcoef * cbodcon
