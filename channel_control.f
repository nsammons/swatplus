	  ...
	  if (bsn_cc%i_subhw == 0 .and. inum1 == inum2) then
          if (bsn_cc%event < 3) then
            if (rtwtr > 0. .and. rchdep > 0.) then
              sedrch  = ob(icmd)%hd(1)%flo * (1. - rnum1)
	        rch_san = ob(icmd)%hd(1)%san * (1. - rnum1)
	        rch_sil = ob(icmd)%hd(1)%sil * (1. - rnum1)
	        rch_cla = ob(icmd)%hd(1)%cla * (1. - rnum1)
	        rch_sag = ob(icmd)%hd(1)%sag * (1. - rnum1)
	        rch_lag = ob(icmd)%hd(1)%lag * (1. - rnum1)
	        rch_gra = ob(icmd)%hd(1)%grv * (1. - rnum1)
            end if
          else
            do ii = 1, nstep
              if (hrtwtr(ii) > 0. .and. hdepth(ii) > 0.) then
                hsedyld(ii) = ts(ii)%hh(inum2)%sed * (1. - rnum1)
