...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do ii = 1, nstep
        if(rainsub(ihru,ii)/bsn_prm%dts > 0.017) then 
           ...
           hrnopcp(sb,ii) = hrnopcp(sb,ii-1) + bsn_prm%dts / 60.
           !! the potential for initial abstraction from paved surface increases based on evaporation
  !         abstinit = min(bsn_cc%abstr,abstinit + pet_day / nstep)   <--commented line bc of 'bsn_cc%abstr' variables 020314 nbs
        end if
      end do
