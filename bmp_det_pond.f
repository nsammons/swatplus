...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &        evap_vol,seep_vol,warea,pi,qovmax,qaddon,depaddon
      ...
      if (time%yrc<dtp_iyr(sb) .or. 
     &(time%yrc==dtp_iyr(sb) .and. i_mo<dtp_imo(sb))) then
         return
      endif
     ...
     &           / (3.* ch_hyd(sb)%s) * pi !m3
      ...
      do ii=1,nstep
         ...
         !qin = ts(ii)%hh(ihout)%flo + qpnd !m^3
         !sedin = ts(ii)%hh(ihout)%sed + sedpnd  !tons
         if (qin>1e-6) then
     ...
     & 	               sqrt(19.6 * watdepact) !m3/s/unit
                  qstage = qstage * dtp_numweir(sb) * 60. * 
     &                   bsn_prm%dts !m^3	         
               else
               !! Partially submerged
     ...
     &             dtp_totwrwid(sb) * (watdepact ** 1.5) * 60. * 
     &        bsn_prm%dts !m3/s
            qout = qout + qstage 
          end if
     ...
     &                  dtp_intcept(sb)
         ...
         if (qout>qin) then !no detention occurs
            ...
            seep_vol = 10.0 * ch_k(2,sb) * seep_sa * 
     &        bsn_prm%dts / 60. !m^3
         ...
         sedpnd = max(0.,sedin - sedout) !tons
   	   ...
   	   !ts(ii)%hh(ihout)%flo = max(0.,qout)
   	   !ts(ii)%hh(ihout)%sed = max(0.,sedout)
