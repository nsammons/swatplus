...
!!	   iterate for subdaily flow/sediment routing
      do ii=1,nstep
         ...
         qout = 0.; spndconc = 0.
         !qin = ts(ii)%hh(ihout)%flo   !m^3
         qpnd =  qin + qpnd !m^3
         !sedin = ts(ii)%hh(ihout)%sed !tons
...
!!                Discharge out of extended detention storage through inverted PVC pipe
     ...
     &              hdep_ext,wtp_pmann(sb),wtp_ploss(sb),qout) !m3/s
            ...
            qout = qout * bsn_prm%dts * 60. !m3/s ->m^3
...
!!       Seepage, evaporation and rainfall
     ...
     &      b1 ** 0.5 + 5.64 * qdepth ** 2 * b1 * wtp_sdslope(sb) !m2
         ...
         seep = wtp_k(sb) / 1000. / 60. * bsn_prm%dts * seepa !m3
...
!!       Sediment removal
     ...
     &        tmpw + 1.7517		
         ...
         if(qout>0.1) then
            !WERF equation
            qhyd = qout / bsn_prm%dts / 60. / surfa * 100.  ! m3/s / m2 * 100 = cm/s
     ...
     &		      (qhyd * wtp_hydeff(sb))) ** (-wtp_hydeff(sb)))  ! mg/l
            ...
            decayexp = exp(-7.667e-3 * bsn_prm%dts / 60. * dp)
