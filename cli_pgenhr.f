...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      pkrr = altc * wst(iwst)%weat%precip / tconc(jj)           !! mm/h
     ...
     &                       (1. - Exp((rtp - Real(pt)) / (60. * xkp2)))
      ...
      !! at end of storm
      if (wst(iwst)%weat%precip > sumrain .and. itime <= nstep) then
        rainsub(jj,itime) = wst(iwst)%weat%precip - sumrain
      end if
