...
!! Calculate drainage area of vfs 1 2 3 in ha
	drain_vfs1 = (1-vfscon(j))* hru(j)%ha
	drain_vfs2 = ((1-vfsch(j)) * vfscon(j))* hru(j)%ha
	drain_vfs3 = vfscon(j) * vfsch(j) * hru(j)%ha

!! Calculate area of vfs 1 and 2 in ha
	area_vfs1 = hru(j)%ha * 0.9 / vfsratio(j)
	area_vfs2 = hru(j)%ha * 0.1 / vfsratio(j)
...
!! calculate sediment loading over buffer area in kg/m^2
	vfs_sed1 = (sedyld(j) / hru(j)%ha * 1000 * drain_vfs1) /
     & (area_vfs1 * 10000)
	vfs_sed2 = (sedyld(j) / hru(j)%ha * 1000 * drain_vfs2) /
     & (area_vfs2 * 10000)
