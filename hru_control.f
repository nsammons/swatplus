...
!        if (pot(j)%vol < 1.e-6) call etact
     ...
     &    smx(j)) - precipday + qday + latq(j) + sepbtm(j) + qtile
        ...
        if (time%day == wst(iwst)%mseas_dayf) pcom(j)%mseas = 1
        if (pcom(j)%mseas == 1) then
          if (time%day >= wst(iwst)%mseas_dayf .and. time%day <= 
     &                                        wst(iwst)%mseas_daye) then
            sol_stup2 = soil(j)%phys(1)%st + soil(j)%phys(2)%st
            sol_fcup2 = soil(j)%phys(1)%fc + soil(j)%phys(2)%fc
            do ipl = 1, npl(j)
              idp = pcom(j)%plcur(ipl)%idplt
              if (sol_stup2 >= pldb(idp)%frsw_gro * sol_fcup2) then
                call mgt_trop_gro
              end if
            end do
          else if (time%day == wst(iwst)%mseas_daye) then
            call mgt_trop_gro
          end if
        end if
