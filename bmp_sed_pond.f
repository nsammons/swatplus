...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &           bsn_prm%dts * 60.
            ...
            qtrns = ksat * tsa / 1000./ 60. * bsn_prm%dts
            qpnd = qpnd - qtrns
            If (qpnd<0) then
              qpnd = 0.
              qtrns = 0.
            ...
            qevap = tsa * sub_etday(sb) / 1000. / 1440. * 
     &         bsn_prm%dts !m^3
...
!            If (phead>pdia/2.) then
               qpipe = pipeflow(pdia,phead) * bsn_prm%dts * 60. !m^3 
!            else
!               qpipe = qout *  0.9 
!            endif
...
!       write(*,'(3i5,20f10.3)') time%yrc,iida,ii,precipdt(ii),qin,
