	...
	do k = 1, nstep
     ...
     &                                 (urbdb(ulu)%thalf + twash(j))
          ...
          !! calculate wash-off of solids
              ...
              dirt=dirto * Exp (- urbk * real(bsn_prm%dts) / 60.)
              if (dirt < 1.e-6) dirt = 0.0

          !! set time to correspond to lower amount of dirt
              twash(j) = 0.
              twash(j) = urbdb(ulu)%thalf * dirt                        
     &                     / (urbdb(ulu)%dirtmx - dirt)
              sus_sol = 0.
              tn = 0.
              tp = 0.
              tno3 = 0.
          !! amounts are kg/ha
              ...
              ubntss(k) = (.001*sus_sol*hru(j)%ha)*urbdb(ulu)%fimp
     ...
     &                                        (1. - urbdb(ulu)%fimp)
          ...
          !! no surface runoff
              twash(j) = twash(j) + bsn_prm%dts / 1440.
