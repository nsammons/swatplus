      ...
      type basin_control_codes
        character(len=13) :: name
        character(len=13) :: petfile     !! potential et filename
        character(len=13) :: wwqfile     !! watershed stream water quality filename
      ...
      type basin_parms
        ...
        real :: r2adj = 1.          !! curve number retention parameter adjustment for low gradient
                                    !!  non-draining soils
        integer :: igen = 0         !!  random generator code: 
                                    !!   0 = use default numbers
                                    !!   1 = generate new numbers in every simulation 
        integer :: dts = 0          !!  rainfall data time step
      ...
      type basin_print_codes
        ...
        integer :: sout = 0           !!  soils output file (soils.out)
        integer :: mout = 0           !!  management output file (mgt.out)
      end type basin_print_codes
      type (basin_print_codes) :: pco
           
      contains
      include 'basin_cc_read.f'
      include 'basin_prm_read.f'
      include 'basin_prm_default.f'
      include 'basin_print_codes_read.f'
      include 'basin_output.f'
