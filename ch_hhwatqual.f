...
!! hourly loop
      do ii = 1, nstep
       ...
       if (hrtwtr(ii) / (bsn_prm%dts * 60.) > 0.01) then
!! concentrations
         ...
         cinn = 0.
         if (wtrin > 0.001) then
         chlin = 1000. * ts(ii)%hh(inum2)%chla * (1. - rnum1) / wtrin
         algin = 1000. * chlin / ch_nut(jnut)%ai0        !! QUAL2E equation III-1
         orgnin = 1000. * ts(ii)%hh(inum2)%orgn * (1. - rnum1) / wtrin
         ammoin = 1000. * ts(ii)%hh(inum2)%nh3 * (1. - rnum1) / wtrin
         nitritin = 1000. * ts(ii)%hh(inum2)%no2 * (1. - rnum1) /       
     &                                                             wtrin
         nitratin = 1000. * ts(ii)%hh(inum2)%no3 * (1. - rnum1) / wtrin
         orgpin = 1000. * ts(ii)%hh(inum2)%sedp * (1. - rnum1) / wtrin
         dispin = 1000. * ts(ii)%hh(inum2)%solp * (1. - rnum1) / wtrin
         cbodin = 1000. * ts(ii)%hh(inum2)%cbod * (1. - rnum1) / wtrin
         disoxin= 1000. * ts(ii)%hh(inum2)%dox * (1. - rnum1) / wtrin
         end if
         ...
         if (ii == 1) then
         algcon = (algin * wtrin + ch(jrch)%algae * hrchwtr(ii)) /      
     &                                                            wtrtot
...
!! set end of day concentrations
      ch(jrch)%algae = halgae(nstep)
      ch(jrch)%chlora = hchla(nstep)
      ch(jrch)%organicn = horgn(nstep)
      ch(jrch)%ammonian = hnh4(nstep)
      ch(jrch)%nitriten = hno2(nstep)
      ch(jrch)%nitraten = hno3(nstep)
      ch(jrch)%organicp = horgp(nstep)
      ch(jrch)%disolvp = hsolp(nstep)
      ch(jrch)%rch_cbod = hbod(nstep)
      ch(jrch)%rch_dox = hdisox(nstep)
