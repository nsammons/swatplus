...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do j = 1, mhru
        ...
        do ipl = 1, npl(j)
          ...
          hru(j)%rsd_flt(ipl)%nmass = 0.43 * hru(j)%rsd_flt(ipl)%mass 
     &                                                             / 57.
          hru(j)%rsd_flt(ipl)%pmass = 0.43 * hru(j)%rsd_flt(ipl)%mass 
     &                                                            / 300.
     ...
     &               exp(pldb(idp)%pop1 - pldb(idp)%pop2 * xx))
      ...
      do ihru = 1, mhru
        iluse_db = hru(ihru)%dbs%landuse    !points to landuse.lu
        hru(ihru)%luse%cn_lu = luse_db(iluse_db)%cn_lu
        hru(ihru)%luse%usle_p = luse_db(iluse_db)%usle_p
        hru(ihru)%luse%urb_lu = luse_db(iluse_db)%urb_lu
        hru(ihru)%luse%ovn = luse_db(iluse_db)%ovn
      end do
      ...
      do ihru = 1, mhru
        ...
        ifield_db = topo_db(itopohd_db)%field_db
        ihyd_db = hru(ihru)%dbs%hyd
        ...
        hru(ihru)%hyd%evpot = hyd_db(ihyd_db)%evpot
        ...
        ! set field data
        hru(ihru)%field%len = field_db(ifield_db)%len
        ...
        hru(ihru)%topo%field_db = topo_db(itopohd_db)%field_db
        hru(ihru)%topo%channel_db = topo_db(itopohd_db)%channel_db
      end do
      
      !!Section 1
      !!this section sets, allocates, and initializes the original soil database
      msoils = amax1 (0, msoils)
