      ...
      type year_hyd
        type (hyd_output), dimension(:),allocatable :: hmon
        type (hyd_output), dimension(:),allocatable :: hyr
      end type year_hyd
      type (year_hyd), dimension(:),allocatable :: yr
                  
      type month_hyd
        type (year_hyd), dimension(:),allocatable :: yr
      end type month_hyd
      type (month_hyd), dimension(:),allocatable :: mn

      type object_connectivity
        character(len=16) :: name = "hru_default"
        integer :: typ = 1       !object type - ie hru, hru_lte, sub, chan, res, recall
        integer :: props = 1     !properties number from data base (ie hru.dat, sub.dat)
        integer :: props2        !additional properties number from data base (ie chan wq)
        ...
        integer, dimension(:), allocatable :: obtyp_in     !inflow object type (ie 1=hru, 2=sd_hru, 3=sub, 4=chan, etc)
      ...
      type spatial_objects
        ...
        integer :: recday = 0    !8-number of recdays's or 1st recday command
        integer :: recmon = 0    !9-number of recmon's or 1st recmon command
        integer :: recyr = 0     !10-number of recyr's or 1st recyr command
      ...
      contains
      include 'hyd_allo.f'
      include 'hyd_connect_out.f'
      include 'hydin_output.f'
      include 'hydout_output.f'
      include 'obj_output.f'
      include 'object_output_read.f'
      include 'hyddep_output.f'
            
      !! function to convert concentration to mass
      subroutine hyd_convert (hyd1)
