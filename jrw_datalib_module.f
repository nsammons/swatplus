      ...
      type data_files_max_elements
        integer :: topo = 1           !!nubz
        integer :: hyd = 1            !!nubz
        integer :: soil = 1           !! none     |number of types of soils 
        integer :: landuse = 1        !! none     |number of landuse types
        integer :: mgt_ops = 1        !! none     |number of records in management
        integer :: pothole = 1        !! none     |number of potholes
        integer :: sdr = 1            !! none     |number of types of susbsurface drain systems
        integer :: str_ops = 1        !! none     |number of management ops
        integer :: septic = 1         !! none     |number of types of septic systems
        integer :: plantcom = 1       !! none     |number of plant communities
        integer :: soiltest = 1       !! none     |number of soiltest 
        integer :: pestdb = 1         !! none     |number of pesticide ops
        integer :: bactdb = 1         !! none     |number of bacteria props
        integer :: sno = 1            !! none     |number of snow props
        integer :: atmodep = 1        !! none     |atmospheric deposition props
        integer :: autofertop_db = 1  !! none     |auto fertilization operations
        integer :: airrop_db = 1      !! none     |auto irrigation operations
        integer :: contfertop_db = 1  !! none     |continuous fertilization operations
        integer :: contpestop_db = 1  !! none     |continuous pesticide operations
        integer :: fertop_db = 1      !! none     |fertlizer operations
        integer :: grazeop_db = 1     !! none     |grazing operations
        integer :: hkop_db = 1        !! none     |harvest kill operations
        integer :: harvop_db = 1      !! none     |harvest only operations
        integer :: irrop_db = 1       !! none     |irrigation operations
        integer :: sweepop_db = 1     !! none     |sweep operations
        integer :: terrop_db = 1      !! none     |terrace data
        integer :: contop_db = 1      !! none     |contour data
        integer :: filtop_db = 1      !! none     |filter strip data
        integer :: stripop_db = 1     !! none     |strip cropping data
        integer :: fireop_db = 1      !! none     |fire data
        integer :: grassop_db = 1     !! none     |grassed waterways data
        integer :: plparmop_db = 1    !! none     |plant parms update data
        integer :: rsdmgtop_db = 1    !! none     |residue mangement data
        integer :: bmpuserop_db = 1   !! none     |user defined upland CP removal
        integer :: cond = 1           !! none     |conditional data
      end type data_files_max_elements
      type (data_files_max_elements), save :: db_mx
          
 !     type data_files            
 !       character(len=15) :: fertdbase = ' fertilizer.frt'
 !       character(len=15) :: pestdbase = '  pesticide.pst'
 !       character(len=15) :: urbandb =   '      urban.urb'
 !       character(len=15) :: tilldbase = '    tillage.til'
 !       character(len=15) :: septdb =    '     septic.sep'
 !       character(len=15) :: plantdb =   '      plant.plt'
 !       character(len=15) :: atmodb =    '       atmo.atm'
 !     end type data_files
 !     type (data_files), save :: dbase_files
      ...
      type aquifer_database
        ...
        real :: revap = 0.        !! mm H2O        |threshold depth of water in shallow aq req to allow revap to occur
      ...
      type pesticide_db
        character(len=17) :: pestnm   !!                     |pesticide name
      ...
      type tillage_db
        ...
        real :: ranrns = 0.          !! mm                 |random roughness for a given HRU
      end type tillage_db
      ...
      type urban_db
        ...
        real :: curbden = 0.0      !! km/ha             |curb length density in HRU                 
...
!!                                         | if IRRSC=5, not used
      ...
      type (irrigation_operation), dimension(:), allocatable ::         
     &                                                 irrop_db
...
!!                                         | if IRRSC=5, not used
      ...
      type (autoirrigation_operation), dimension(:), allocatable ::     
     &                                                      airrop_db
      ...
      type (filtstrip_operation), dimension(:), 
     &                                      allocatable :: filtstrip_db
      ...
      type (stripcrop_operation), dimension(:), 
     &                                      allocatable :: stripcrop_db
      ...
      type (grwaterway_operation),dimension(:), 
     &                                    allocatable :: grwaterway_db
...
!!! never switched from array to data type in....needs to be cleaned up !!!!
      ...
      type (autofertilizer_operation), dimension(:), allocatable ::
     &                                                    autofertop_db
      ...
      type (continuousfertilizer_operation),dimension(:), allocatable ::
     &                                                    contfertop_db
      ...
      type (continuous_pesticide_operation),dimension(:), allocatable ::
     &                                                  contpestop_db
...
!!                                                       becomes residue on the soil surface
      ...
      type (streetsweep_operation), dimension(:), allocatable ::        
     &                                                        sweepop_db
      ...
      type management_ops
        ...
        integer :: op = 0         !! |none        |operation code number
          !! 1 == establish plant community | 2 == plant | 3 == harvest only
          !! 4 == kill | 5 == harvest and kill | 6 == tillage | 7 == irrigation
          !! 8 == auto irrigation | 9 == release/impound | 10 == fertlilizer
          !! 11 == auto fertilizer | 12 == continuous fertilizer | 13 == pesticide application
          !! 14 == continuous pesticide | 15 == grazing | 16 == burning
          !! 17 == street sweeping | 18 == print plant variables | 20 == skip to end of year
        integer :: mon = 0
        integer :: day = 0
        integer :: jday = 0
        integer :: year = 0
        ...
        integer :: op3 = 0        !! |none          |application amount (mm or kg/ha)
      ...
      type structural_practices
        ...
        integer, dimension(11) :: str_prac = 0
        ! 1 == terrace; 2 == tile; 3 == contouring; 4 == filter strip; 5 == strip cropping
        ! 6 == fire; 7 == grass wway; 8 == plant parm update; 9 == residue management; 10 == user defined
        ! 11 == septic system
      ...
      type topography_db
        ...
        integer :: route_db=0     !!               |              |pointer to channel.dat
      end type topography_db
      type (topography_db), dimension (:), allocatable :: topo_db
              
      type fields_db
           character(len=13) :: name = 'default'
           real :: len = 500. !!               |m             |field length for wind erosion
      ...
      type hydrology_db
         ...
         real :: erorgp = 0.     !! erorgp(:)     |none          |organic P enrichment ratio, if left blank
                                 !!                              |the model will calculate for every event
         real :: evpot =0.       !! evpot(:)      |none          |pothole evaporation coefficient
         ...
         real :: harg_petco = .0023  !!           |              |coefficient related to radiation used in 
                                     !!                            Hargreaves equation
         real :: cncoef = 0.3        !!           |              |plant ET curve number coefficient
       end type hydrology_db
        type (hydrology_db), dimension (:),                  
     &                                        allocatable :: hyd_db
      ...
      type routing_nut_data       ! used for 2-stage ditch in chandeg and overland flow
        ...
        real :: ts_int = 1.312    ! ppm             |intecept of total P reduction equation
...
!      include 'data_files_read.f'
      include 'bac_lsparms_read.f'
      include 'fertparm_read.f'
      include 'pestparm_read.f'
      include 'plantparm_read.f'
      include 'plantparm_init.f'
      include 'tillparm_read.f'
      include 'urbanparm_read.f'
      include 'septicparm_read.f'
      include 'mgt_mgtops_read.f'
      include 'mgt_irrops_read.f'
      include 'mgt_autoirrops_read.f'
      include 'mgt_fertops_read.f'
      include 'mgt_autofertops_read.f'
      include 'mgt_contfertops_read.f'
      include 'mgt_pestops_read.f'
      include 'mgt_contpestops_read.f'
      include 'mgt_harvops_read.f'
      include 'mgt_grazeops_read.f'
      include 'mgt_sweepops_read.f'
      include 'str_init_read.f'
      include 'scen_terrace_read.f'
      include 'scen_contour_read.f'
      include 'scen_filtstrip_read.f'
      include 'scen_stripcrop_read.f'
      include 'scen_fire_read.f'
      include 'scen_grwway_read.f'
      include 'scen_plparmup_read.f'
      include 'scen_rsdmgt_read.f'
      include 'scen_bmpuser_read.f'
      include 'sep_read.f'
      include 'solt_db_read.f'
      include 'topo_read.f'
      include 'field_read.f'
      include 'hydrol_read.f'
      include 'landuse_read.f'
      include 'cntbl_read.f'
      include 'sdr_read.f'
!      include 'potdb_read.f'
      include 'snowdb_read.f'
      include 'soil_db_read.f'
      include 'atmoparm_read.f'
      include 'aqu_read.f'
      include 'res_hyd_read.f'
      include 'res_sed_read.f'
      include 'res_nut_read.f'
      include 'res_pst_read.f'
      include 'res_weir_read.f'
      include 'ch_read_hyd.f'
      include 'ch_read_sed.f'
      include 'ch_read_nut.f'
      include 'ch_read_pst.f'
      include 'rte_read_nut.f'
