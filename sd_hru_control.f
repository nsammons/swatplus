...
!           IF ave temp > 0  compute runoff                             
            ...
            s = sd(isd)%s1 + sd(isd)%sw / sd(isd)%awc *                 
     &             (sd(isd)%s3-sd(isd)%s1)
            cn_sd = 25400. / (s + 254.)
...
!         begin growth for tropical plants - Strauch, Volk, et al.
          IF (pldb(iplt)%idc == 8) then
            if (time%day == sd_db(isd_db)%igrow1) sd(isd)%igro = 0
            if (time%day >= sd_db(isd_db)%igrow1 .and. time%day <= 
     &                                        sd_db(isd_db)%igrow2) then
              if (sd(isd)%igro == 0 .and. sd(isd)%sw > 
     &            pldb(iplt)%frsw_gro * sd(isd)%awc) then
                sd(isd)%igro = 1
                sd(isd)%g = 0.
                sd(isd)%alai = 0.
                sd(isd)%dm = 0.
              ...
              if (sd(isd)%igro == 0 .and. time%day == 
     &                                        sd_db(isd_db)%igrow2) then
                  
          ! calc yield, print max lai, dm and yield
          if (pco%mout == 1) then
            yield = sd(isd)%dm * pldb(iplt)%hvsti
            write (4700,*) isd, time%day, time%yrc, pldb(ic)%plantnm,
...
!           irrigate IF water stress is < 0.7                             
            ...
            IF (tave .gt.pldb(iplt)%t_base) THEN
              tgx = 2. * pldb(iplt)%t_opt -
     &                 pldb(iplt)%t_base - tave
              rto = ((pldb(iplt)%t_opt - tave )
     &            /(tgx+1.e-6))**2
              IF (rto.le.200.) THEN 
            ...
            f = sd(isd)%g / (sd(isd)%g + EXP(plcp(iplt)%leaf1 -
     &                                  plcp(iplt)%leaf2 * sd(isd)%g))
            ff = f - sd(isd)%hufh 
            sd(isd)%hufh = f 
            deltalai = ff * pldb(iplt)%blai * (1.0-
     &        EXP(5.0 *(sd(isd)%alai-pldb(iplt)%blai)))
     &                                                    * sqrt(reg)
...
!         adjust actual et for growing season                           
          ...
          IF (sd(isd)%sw.gt.sd(isd)%awc) THEN 
            ...
            flowlat = .048 * swf * sd(isd)%sc*sd_db(isd_db)%slope /
     &            sd_db(isd_db)%slopelen
          ...
          sd(isd)%gwflow = sd(isd)%gwflow * sd_db(isd_db)%abf +
     &         perc * (1. - sd_db(isd_db)%abf)
...
!!        compute sediment yield with MUSLE
          sedin = (runoff * peakr * 1000. *                             
     &         sd_db(isd_db)%dakm2) ** .56 * sd(isd)%uslefac 
	    !! add subsurf sediment - t=ppm*mm*km2/1000.
	    qssubconc = 500.
	    qssub = qssubconc * (flowlat + sd(isd)%gwflow) *    
     &          sd_db(isd_db)%dakm2 / 1000.
	    sedin = sedin + qssub
          ...
          do k = 1, 1000
            if (iv < peakrbf) then
              ...
              if (iv < peakr) then
                ...
                tb = (peakr - (iv - intval)) * (1.5 *                   
     &             sd_db(isd_db)%tc) / peakr
